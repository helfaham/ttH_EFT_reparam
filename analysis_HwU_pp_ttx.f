...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,1
       ...
       call HwU_book(l+21,'higgs pT    '//cc(i),100,0.d0,500.d0)
      enddo
      return
      end
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      integer i,kk,l
      double precision www,xptq(0:3),xptb(0:3),yptqtb(0:3),xh(0:3),ycut,ptcut 
     $     ,ptq1,ptq2,pth,ptg,yq1,yq2,etaq1,etaq2,azi,azinorm,qqm,dr,yqq 
...
c
      ...
      do i=0,3
        xptq(i)=p(i,3)
        xptb(i)=p(i,4)
        if (ihiggs.ne.0) xh(i)=p(i,ihiggs) !HF for the higgs
...
c
c      if(ddflag)then
c         call HwU_fill(l+1,ptg,WGTS)
c         call HwU_fill(l+18,ptg,WGTS)
c         if(ptg.gt.0) call HwU_fill(l+2,log10(ptg),WGTS)
c         call HwU_fill(l+3,qqm,WGTS)
c         call HwU_fill(l+4,azi,WGTS)
c         call HwU_fill(l+13,azi,WGTS)
c         if(azinorm.gt.0) call HwU_fill(l+17,log10(azinorm),WGTS)
c         call HwU_fill(l+5,dr,WGTS)
c         call HwU_fill(l+14,dr,WGTS)
c         call HwU_fill(l+10,etaq1-etaq2,WGTS)
c         call HwU_fill(l+11,yqq,WGTS)
c         call HwU_fill(l+12,yq1-yq2,WGTS)
c      endif
c      if(abs(yq2).lt.ycut)then
c         call HwU_fill(l+6,ptq2,WGTS)
c         call HwU_fill(l+19,ptq2,WGTS)
c         if(ptq2.gt.0) call HwU_fill(l+7,log10(ptq2),WGTS)
c      endif
c      if(ptq2.gt.ptcut)call HwU_fill(l+15,yq2,WGTS)
c      if(abs(yq1).lt.ycut)then
c         call HwU_fill(l+8,ptq1,WGTS)
c         call HwU_fill(l+20,ptq1,WGTS)
c         if(ptq1.gt.0) call HwU_fill(l+9,log10(ptq1),WGTS)
c      endif
c      if(ptq1.gt.ptcut)call HwU_fill(l+16,yq1,WGTS)
 999  return      
      end
